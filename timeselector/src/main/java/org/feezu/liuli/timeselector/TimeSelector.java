...
public class TimeSelector {
    ...
    private void initDialog() {
        if (seletorDialog == null) {
            ...
            window.setWindowAnimations(R.style.SingleDialogWindowAnim);
    ...
    private void addListener() {
        ...
        month_pv.setOnSelectListener(new PickerView.onSelectListener() {
            ...
            public void onSelect(String text) {
                ...
                //因为每个月的时间可能不同，如8月31号滑动到9月份，在没设置日的情况下，就变成了9月31号，9月没有31号，就会变成10月份，
                //所以在每次月份变动时，先把日归1
    ...
    private void loadComponent() {
        ...
        if (canShowCustomerDate()) {
            String[] date = DateUtil.format(showDate, DateUtil.dateFormatYMD).split("-");
            year_pv.setSelected(date[0]);
            month_pv.setSelected(date[1]);
            day_pv.setSelected(date[2]);
        } else {
     ...
     * 显示客户自定义时间
     */
    private boolean canShowCustomerDate() {
        if (showDate != null) {
            if (startCalendar.getTime().getTime() > showDate.getTime() && showDate.getTime() > endCalendar.getTime().getTime()) {
                Toast.makeText(context, "显示的时间不在时间区间之内！", Toast.LENGTH_LONG).show();
                return false;
            } else {
                selectedCalender.setTime(showDate);
                return true;
